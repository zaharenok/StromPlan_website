...
export default function LoadingOverlay({ isVisible, isComplete = false }: LoadingOverlayProps) {
  ...
  useEffect(() => {
    ...
    // Total animation: 20 seconds (first 3 steps ~3.75s each, preparing 8.75s)
    const steps: { step: LoadingStep; delay: number }[] = [
      { step: 'uploading', delay: 0 },
      { step: 'sending', delay: 3750 },
      { step: 'analyzing', delay: 7500 },
      { step: 'preparing', delay: 11250 }, // Stays on this step for ~8.75 seconds
    ];
    ...
    // Smooth progress bar - reaches 95% in exactly 20 seconds
    let currentProgress = 0;
    const progressInterval = setInterval(() => {
      currentProgress += 0.475; // 0.475% per 100ms = 4.75% per second = 20 seconds to reach 95%
